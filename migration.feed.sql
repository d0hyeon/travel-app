...
CREATE POLICY "photo_posts_select" ON photo_posts FOR SELECT
  USING (can_view_post(visibility, author_id, trip_id));

CREATE POLICY "photo_posts_insert" ON photo_posts FOR INSERT
  WITH CHECK (author_id = auth.uid());

CREATE POLICY "photo_posts_update" ON photo_posts FOR UPDATE
  USING (author_id = auth.uid())
  WITH CHECK (author_id = auth.uid());

CREATE POLICY "photo_posts_delete" ON photo_posts FOR DELETE
  USING (author_id = auth.uid());
