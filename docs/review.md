...
설계 판단은 `docs/design.md`, 표현 판단은 `docs/expression.md`를 기준으로 삼고, 이 문서는 작업 후 빠뜨린 위험을 마지막으로 확인하는 용도다.
...
### 변경 영향
- [ ] 이 변경이 어떤 사용자 흐름에 영향을 주는지 설명할 수 있는가
- [ ] 함께 바뀌어야 하는 타입·훅·UI·테스트가 빠지지 않았는가
- [ ] 기존 호출부가 새 인터페이스를 오해 없이 사용할 수 있는가
...
- [ ] 조건식과 중간 값이 읽는 사람의 추론을 줄이는가

### 상태와 예외
- [ ] 로딩·빈 상태·에러 상태 중 이 변경이 건드리는 상태를 확인했는가
- [ ] undefined/null/빈 배열 같은 경계값이 자연스럽게 처리되는가
- [ ] 내부에서 관리할 상태를 호출부에 떠넘기고 있지는 않은가

### 검증
- [ ] 변경된 사용자 흐름을 직접 실행하거나 테스트로 확인했는가
- [ ] 자동 검증을 돌리지 않았다면, 확인하지 못한 위험을 설명할 수 있는가
