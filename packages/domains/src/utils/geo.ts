...
export function isOverseasByCoordinate(
  ...args: [number, number] | [Coordinate]
): boolean {
  if (args.length === 2) {
    const [lat, lng] = args;

    return isKoreaCoordinate(lat, lng) === false;
  }
  return isKoreaCoordinate(args[0].lat, args[0].lng) === false;
}
