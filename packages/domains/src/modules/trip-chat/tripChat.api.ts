...
import { getUserProfileById } from "@waylog/domains/modules/user-profile";
...
export async function getChatMessages(
  tripId: string,
): Promise<Omit<ChatMessage, "profile">[]> {
...
export async function sendChatMessage(
  ...
  if (profile == null)
    throw new Error("사용자 데이터가 올바르게 설정되지 않았습니다.");
  ...
  supabase.functions
    .invoke("chat-web-push", {
      body: {
        tripId,
        senderId: user.id,
        body: `${profile.name}: ${content.length > 50 ? content.slice(0, 50) + "…" : content}`,
      },
    })
    .catch(() => {});
