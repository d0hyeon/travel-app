...
describe('trip-chat 통합', () => {
  describe('메시지 목록 조회', () => {
    ...
    it('메시지가 created_at 오름차순으로 정렬된다', async () => {
      ...
      const times = result.current.messages.map((m) => m.createdAt)
  ...
  describe('메시지 전송', () => {
    ...
    it('서버 오류 시 isPending이 false로 돌아온다', async () => {
      ...
      const { result } = renderHook(() => useSendChatMessage(MOCK_TRIP_ID), { wrapper })
      act(() => { result.current.send('실패할 메시지') })
      await waitFor(() => expect(result.current.isPending).toBe(false))
    })
  })
    ...
    it('setQueryData로 새 메시지를 즉시 추가할 수 있다', async () => {
      ...
      act(() => {
        queryClient.setQueryData(
          ['trip_messages', 'list', MOCK_TRIP_ID],
          (prev: typeof newMessage[]) => [...(prev ?? []), newMessage]
        )
      })

      await waitFor(() => expect(result.current.messages).toHaveLength(3))
      expect(result.current.messages[2].content).toBe('실시간 메시지')
    })
  })
})
