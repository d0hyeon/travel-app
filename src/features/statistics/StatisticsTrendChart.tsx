...
export function StatisticsTrendChart({ data, viewMode = 'line' }: StatisticsTrendChartProps) {
          ...
          cases={{
            'bar': () => (
              <BarChart data={chartData} margin={{ top: 12, right: 20, left: 8, bottom: 12 }} barCategoryGap={18}>
                ...
                <Bar dataKey="amountInKRW" fill={theme.palette.success.main} radius={[8, 8, 0, 0]} maxBarSize={36} />
              </BarChart>
            ),
            'donut': () => (
              <StatisticsDonutChart
                data={chartData
                  // .toSorted((a, b) => b.amountInKRW - a.amountInKRW)
                  .map((item) => ({
                    id: item.axisId,
                    label: item.tripName,
          ...
          defaultComponent={() => (
            <LineChart data={chartData} margin={{ top: 12, right: 20, left: 8, bottom: 12 }}>
              ...
              <Line
                ...
                strokeWidth={3}
                dot={{ r: 4, fill: theme.palette.success.main, stroke: '#fff', strokeWidth: 2 }}
                activeDot={{ r: 5 }}
              />
            </LineChart>
          )}
